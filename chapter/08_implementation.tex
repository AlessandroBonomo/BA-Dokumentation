...
\textbf{Auswertung:} \\
Erste Durchführung: \\
Bei einer ersten Durchführung wurde festgestellt, dass noch einige Probleme vorliegen. Wie in der Abbildung \ref{performance_tests_vorher} zu sehen ist, benötigte die Applikation bei mehr als einem Drittel der eingegangen Anfragen mehr als eine Sekunde um darauf zu antworten. Bei beinahe einem Drittel wurde sogar gar keine Antwort gesendet. Das Problem konnte darauf zurück geführt werden, dass bei jeder Anfrage ebenfalls eine Anfrage an Bootstrap, etc. gesendet wurde.

	\begin{figure}[H]
		\includegraphics[width=\textwidth, height=\textheight, keepaspectratio]{images/performance_uebersicht_vorher.png}
		\caption{Performance Test Übersicht vorher}
			\label{performance_tests_vorher}
	\end{figure}

Massnahmen: \\
Anhand der Erkentnisse der ersten Durchführung, wurden Bibliotheken, wie zum Beispiel 'jquery-3.1.1.min.js' und 'bootstrap.min.css', welche für die Darstellung der Seiten benötigt werden, statisch in die Applikation eingebunden.


Zweite Durchführung: \\
Insgesamt wurden über einen Zeitraum von 1 Minute und 51 Sekunden 38'909 Requests abgesetzt. In der Abbildung \ref{request_per_second} ist ersichtlich, wie die Requests über den erwähnten Zeitverlauf verteilt absetzt wurden.
...
Wie in der Abbildung \ref{performance_tests} ersichtlich ist, benötigte die Applikation bei etwa 5\% der eingegangenen Anfragen etwas länger als eine Sekunde für die Antwort. Im Vergleich zur ersten Durchführung ist dies eine enorme Verbesserung.
